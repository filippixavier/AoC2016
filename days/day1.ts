...
function star1(input: Direction[]) {
  ...
  for (const { isLeft, steps } of input) {
    ...
    pos = [
      pos[0] + DIRECTIONS[facing][0] * steps,
      pos[1] + DIRECTIONS[facing][1] * steps,
    ];
  }
  ...
  console.log(
    ...
    } steps away from the Eastern HQ`,
  );
}
...
function star2(input: Direction[]) {
  ...
  console.log(
    `We are ${
      Math.abs(pos[0]) + Math.abs(pos[1])
    } steps away from the first location visited twice: the True Eastern HQ`,
  );
}
